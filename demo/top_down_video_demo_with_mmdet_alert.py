...
LastEditTime: 2022-08-05 20:21:08
'''
...
def getCrossPoints(json_file='/home/lyh/mmpose/tests/data/coco/1.json', kx=0, ky=-0.09467121147881472, P=[960, 540, 1], scale=1.5):
    """寻找交点
        json_file: 分割数据存储文件
        k：斜率
        P：定点坐标
...
def judge2DborderIn(json_file='/home/lyh/mmpose/tests/data/coco/1.json', kx=0, ky=-0.09467121147881472, P=[960, 540, 1], score_threshold=0.3, scale=1.5):
    """判断点是否在二维区域内部
        json_file: 分割数据存储文件
        keypoint：目标点
        kx：水平线斜率
        ...
        return:
            Points:[levelpoint1, levelpoint2, levelpoint3, levelpoint4, verticalpoint1, verticalpoint2]
    """
    Points = getCrossPoints(json_file, kx, ky, P, scale)
    ...
    for frame_id, cur_frame in enumerate(mmcv.track_iter_progress(video)):
        ...
        if frame_id < 500:
            for pose_result in pose_results:
                ...
                flag = True
                for p in pose:
                    flag = bool(p[2] > args.kpt_thr)
                    if flag is False:
                        break
                
                # 只识别面朝前或者面朝后的人
                if args.direction != judgeDirection(pose, args.kpt_thr):
                    continue
                ...
                if flag is False:
                    #头是否歪了
                    Crookedhead = False
                    if pose[0][2] > args.kpt_thr and \
                        pose[1][2] > args.kpt_thr and \
                            pose[2][2] > args.kpt_thr:
                        eye = (pose[1] + pose[2])/2
                    ...
                    out_border = 0
                    for p in pose:
                        if judge2DborderIn(P=p, score_threshold=args.kpt_thr):
                            in_border += 1
                        elif not judge2DborderIn(P=p, score_threshold=args.kpt_thr):
                            out_border += 1
                    ...
                    #内外都有，手在内部，头部歪曲
                    elif judge2DborderIn(P=pose[9], score_threshold=args.kpt_thr) == True and \
                        judge2DborderIn(P=pose[10], score_threshold=args.kpt_thr) == True and \
                            Crookedhead is True and out_border != 0 and in_border != 0:
                            cv2.circle(vis_frame, [int(pose[0][0]),int(pose[0][1])], 5, (0, 0, 255),8)
                            print('warning2: out of border')
                    #内外都有，手部伸展
                    elif out_border != 0 and in_border != 0:
                        if judge2DborderIn(P=pose[9], score_threshold=args.kpt_thr) == False:
                            if abs((pose[9][1]-pose[7][1])/(pose[9][1]-pose[7][1]))<2:
                                p = [int(pose[9][0]),int(pose[9][1])]
                                cv2.circle(vis_frame, p, 5, (0, 0, 255),8)
                                print(args.direction, judgeDirection(pose, args.kpt_thr))
                                print('warning3: out of border')
                        elif judge2DborderIn(P=pose[10], score_threshold=args.kpt_thr) == False:
                            if abs((pose[10][1]-pose[8][1])/(pose[10][1]-pose[8][1]))<2:
                ...
                if len(crossPoints) != 0:
                    ...
                    parallelLineDistance = getDist_P2L_V2(
                        crossPoints[1], 1/k, crossPoints[2])
                    for p in pose:
                        distance1 = getDist_P2L_V2(p, 1/k, crossPoints[1])
                        distance2 = getDist_P2L_V2(p, 1/k, crossPoints[2])
                        if abs(distance1+distance2-parallelLineDistance) > args.tolerable_eer_thr:
            ...
            # cv2.imwrite("demo.jpg", vis_frame)
